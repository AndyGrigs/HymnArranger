...
def suite_plan():
    return [
        ...
        'eighths_ct',         # 3. назад до вісімок, але через тони акорду
        'mixed',              # 4. вісімки з каденціями шістнадцятими — місток
        'sixteenths_arp',     # 5. шістнадцяті арпеджіо — віртуозна варіація
        BASS_LED,             # 6. зміна фактури: мелодію веде ліва рука
        'two_voice_arp',      # 7. два голоси: тема повертається під арпеджіо
        'chord_melody_8va',   # 8. фінал акордами, октавою вище
    ]